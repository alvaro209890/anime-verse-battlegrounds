...
Este documento combina baseline existente e alvo. O repositório possui registry de remotes, envelope v2, `SecurityService` com schemas fechados/replay/rate limit, `TelemetryService` allowlisted, interação com catálogo/alvo/fase fechados, validações de catálogo/serviços e CI com StyLua, Selene, 227 testes de domínio, 62 testes de animação/apresentação, Wally e build Rojo no snapshot canônico `d7c44e8`. Esses checks não provam fuzz no runtime, DataStore real, network ownership, múltiplos servidores, Studio ou dispositivos; os controles restantes precisam ser implementados e medidos por fase.
...
- orçamento de combate de 8 intenções/s, separado do orçamento default de 15/s, com cobertura headless de isolamento entre dois jogadores e limpeza no rejoin;
- defesa e dash podem antecipar VFX e pose local, mas bloqueio, dano, cooldown, custo e movimento continuam decisões do servidor; a comparação visual segue o protocolo `docs/26-VISUAL-VALIDATION-CHECKLIST.md`;
...
- a CI atual usa permissões mínimas e executa StyLua, Selene, 227 testes de domínio, 62 testes de animação/apresentação, fuzz determinístico de segurança, Wally e build Rojo;
