...
Este documento descreve a arquitetura-alvo e distingue o que já existe do que ainda precisa ser entregue. O repositório contém um **esqueleto executável da F0**: bootstrap de servidor/cliente, catálogos, mundo greybox, modelos/animação procedural de NPCs, apresentação local do jogador, receitas visuais de defesa/dash, interação contextual, domínio, save/ProfileStore, controllers de Input/HUD, registry v2, `SecurityService` e `TelemetryService`. A CI versionada executa StyLua, Selene, 235 testes de domínio, 73 testes de animação/apresentação, instalação Wally e build Rojo no snapshot canônico `86228ee`. Isso comprova estrutura e lógica automatizada; **não comprova** roteiro runtime no Studio, DataStore real, teleporte, servidor publicado, mobile ou gamepad.
...
- `ActorAnimator.luau` apresenta NPCs a partir de `EnemyEvent`; `PlayerCombatAnimator.luau` compõe resposta local para leve, pesado, guarda e dash em fases; `AbilityVfx` fornece casca, rastro e impacto de chão locais. Ambos cacheiam joints e nunca alteram root, hitbox ou regra de combate;
...
- os 235 testes de domínio e 73 testes de animação/apresentação cobrem dados, domínio, controllers, interação, apresentação pura e segurança/telemetria, mas ainda não substituem os testes de integração e runtime previstos neste documento.
...
Pipeline atual: StyLua check → Selene → 235 testes de domínio + 73 testes de animação/apresentação → instalação Wally → build Rojo. A evolução aprovada acrescenta type check Roblox e fixtures de migração sem remover os gates existentes. A CI não terá credenciais de produção e não publicará place automaticamente.
