...
Este documento descreve a arquitetura-alvo e distingue o que já existe do que ainda precisa ser entregue. O repositório contém um **esqueleto executável da F0**: bootstrap de servidor/cliente, catálogos, greybox, domínio, save/ProfileStore, controllers de Input/HUD, registry v2, `SecurityService` e `TelemetryService`. A CI versionada executa StyLua, Selene, 155 testes Lune, instalação Wally e build Rojo. Isso comprova estrutura e lógica automatizada; **não comprova** roteiro runtime no Studio, DataStore real, teleporte, servidor publicado, mobile ou gamepad.
...
- os 155 testes Lune cobrem dados, domínio, controllers e segurança/telemetria pura, mas ainda não substituem os testes de integração e runtime previstos neste documento.
...
Pipeline atual: StyLua check → Selene → 155 testes Lune → instalação Wally → build Rojo. A evolução aprovada acrescenta type check Roblox e fixtures de migração sem remover os gates existentes. A CI não terá credenciais de produção e não publicará place automaticamente.
