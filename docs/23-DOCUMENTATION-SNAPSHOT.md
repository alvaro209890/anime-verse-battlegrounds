...
| Estado deste documento | snapshot após auditoria visual executável, matriz de usabilidade e hashes de todos os PNGs conceituais |
| Data do commit | 2026-08-16 |
| Última alteração | `scripts/audit_visual_assets.py`; `docs/assets/visual-inventory.json`; `docs/33-ASSET-USABILITY.md`; paleta extraída travada contra RGB de runtime; previews JPEG que faltavam; CI passa a conferir o inventário; 3 testes novos em `tests/animation.luau` |
...
Nesta rodada, o domínio passou com **241 casos**, a animação/apresentação com **76**, o fuzz de segurança com **67** e a simulação de combate ponta a ponta com **19**. Foram adicionados o inventário visual executável, a matriz `docs/33`, a paleta extraída travada contra o RGB já no código (sem recolorir runtime), previews JPEG que faltavam e a auditoria no CI. O total automatizado atual é **403 casos**. Selene, StyLua, Rojo, a auditoria visual e as quatro suítes Lune passaram. Isso continua sendo evidência de contratos, apresentação pura, integridade de arquivo e árvore de build; não é evidência de boot, colisão, replicação, latência, Android, gamepad, DataStore real, upload de textura/áudio ou qualidade visual final. Nenhuma imagem gerada foi ligada ao place.
