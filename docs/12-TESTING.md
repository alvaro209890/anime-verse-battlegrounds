...
> **Snapshot:** 2026-08-12. `tests/run.luau` declara 67 testes F0 alinhados a `docs/13-F0-SLICE.md`. O HEAD `c6530a8` cobria 64 testes da fronteira; esta revisão trava técnicas no spawn.
...
Em 2026-08-12, no worktree Windows, foram executados: Selene 0.31.0 (0 erros); StyLua 2.5.2 com `--line-endings Windows` nos arquivos alterados; 67 testes Lune 0.10.5. Todos passaram. Essa evidência valida o domínio headless de sessão, combate, zona e **técnicas locked no spawn** (`ProgressionService`). Não substitui o CI do commit nem playtest no Studio. Estilhaços, objetivo 1, persistência das flags, geometria do greybox e HUD continuam **não comprovados**.
...
## 2. Cobertura existente: exatamente 67 testes
...
- **`tests/run.luau`** contém os 67 casos e usa módulos reais de `src/`, com um miniframework de asserts.
...
|---|---|---|
| lint + 67 testes Lune | sintaxe, estilo e comportamento unitário coberto no ambiente simulado | física, replicação, UI, input ou serviços Roblox reais |
...
Os testes abaixo são backlog, não parte dos 67 existentes:
...
- Contar casos pelo resumo pode mascarar erro: a fonte é a quantidade real de chamadas `test(...)` em `tests/run.luau`; nesta versão são 67.
