...
> **Snapshot:** 2026-08-12. `tests/run.luau` declara 124 testes F0 alinhados a `docs/13-F0-SLICE.md`. O commit `b4f484c` cobria 109 testes (camada espacial); esta revisão acrescenta os itens 8–10 do backlog — Cadência Quebrada com eco agendado e Fluxo, Estilhaço Ancorado (elite) com leeching, e Retorno de Pulso com postura.

## 1. Estado da execução

Em 2026-08-12, no acer (Linux), foram executados: Selene 0.31.0 (0 erros, 0 warnings); StyLua 2.5.2 (0 diffs); 124 testes Lune 0.10.5 (0 falhas); `rojo build -o /tmp/avb-build.rbxl` (exit 0).
...
## 2. Cobertura existente: exatamente 124 testes
...
| **ProgressionService** (8) | cadência/pulso locked no spawn; grant comet idempotente e flag desconhecida recusa; leave limpa flags; kill do Estilhaço credita 25 e dummy não credita; retorno decrescente 6×25 → 6×12 → 0; decréscimo por âncora, não global; teto de 800 por sessão, valor negativo e jogador desconhecido; elite 80 com cooldown de 180 s por jogador |
...
- **`tests/run.luau`** contém os 124 casos e usa módulos reais de `src/`, com um miniframework de asserts.
...
|---|---|---|
| lint + 124 testes Lune | sintaxe, estilo e comportamento unitário coberto no ambiente simulado | física, replicação, UI, input ou serviços Roblox reais |
...
Os testes abaixo são backlog, não parte dos 124 existentes:
...
- Contar casos pelo resumo pode mascarar erro: a fonte é a quantidade real de chamadas `test(...)` em `tests/run.luau`; nesta versão são 124.
