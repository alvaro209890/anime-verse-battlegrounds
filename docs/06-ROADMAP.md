...
Este roadmap transforma a visão do jogo em uma sequência de entregas verificáveis e passa a ser a referência de escopo para uma equipe de 1–3 pessoas. A implementação **já começou**: os itens 1–13 do backlog de `docs/13-F0-SLICE.md` estão fechados em código/headless — catálogos, serviços de domínio, ProfileStore, camada cliente F0 e segurança/telemetria mínima. No commit `108be31`, a fundação adicional ganhou pisos coerentes com todos os volumes de zona, interação contextual jogável, telegraph acessível e resposta procedural do personagem local. Em 13/08 (`d0f6f8d`) o combate ganhou skins procedurais anime, animações mais dramáticas, balanceamento de dano, números de dano flutuantes e barras de vida dos inimigos. O snapshot de implementação anterior `d7c44e8` declarava **224 testes de domínio** e **55 testes de animação/apresentação**. Depois vieram a cobertura adversarial, a reforma visual do spawn e, em 15/08 (`docs/31`), a rodada do mundo aberto: decoração dirigida por dados da planície/rotas/cratera, skins de Estilhaço como receita pura com alcance honesto, iluminação do spawn em catálogo e a apresentação local dos cinco sinais da fronteira. Em 15/08 (tarde) veio a rodada de evidência headless (`docs/32`): simulação de combate de ponta a ponta, fuzz adversarial ampliado e o runbook do Studio como dado executável. Em 16/08 o CI Linux virou `scripts/ci.sh`, a simulação de combate passou a cobrir pesado e Ombro Cometa em miss/acerto/guarda, e o atalho de casa (`avb-debug home`) ficou executável com o Studio fechado. Ainda em 16/08 a auditoria visual (`docs/33`, inventário com hashes, `--check` no CI) entrou sem ligar PNG ao place. O estado atual é **241 testes de domínio**, **76 de animação/apresentação**, **67 de fuzz** e **19 de simulação de combate** — 403 casos; Selene, StyLua, auditoria visual e Rojo passaram nesta rodada. Lint, build e testes continuam gates distintos de runtime Roblox.

Esse estado **não** equivale a F0 concluída. O build atual não foi executado em Play: não há evidência registrada de boot/spawn, pisos e prompts reais, Parts/Motor6D, HUD/câmera, telegraph sob latência, DataStore real, servidor publicado, toque, mobile ou gamepad. Também faltam dois clientes simultâneos. O próximo marco do programa é reabrir no Studio o snapshot gerado do commit atual e então executar o Gate W1 (`docs/15-WORLD-PRESENTATION.md`), sem promover o código headless a runtime antes dessa sessão. O atlas visual da expansão de domínio está em `docs/24-DOMAIN-EXPANSION-ILLUSTRATION.md`; o pacote de apresentação de combate está em `docs/25-COMBAT-PRESENTATION-PLAN.md`; o índice e o protocolo visual estão em `VISUAL-REFERENCE-INDEX.md` e `docs/26-VISUAL-VALIDATION-CHECKLIST.md`; o catálogo de assets e habilidades futuras está em `docs/27-FUTURE-ABILITY-ASSET-CATALOG.md`; a reforma visual do spawn está em `docs/28-SPAWN-VISUAL-PASS.md`; a reforma do mundo aberto e das skins de inimigo está em `docs/31-WILD-VISUAL-PASS.md`; o roteiro do dia do Studio está em `docs/32-STUDIO-PLAYTEST-RUNBOOK.md`; a usabilidade das imagens geradas está em `docs/33-ASSET-USABILITY.md`; o plano de melhorar skins F0 só com código e CC0 está em `docs/34-CODE-DRIVEN-SKINS.md`. Esses documentos orientam F0 → F1/F2, mas não alteram o estado de implementação nem substituem validação em runtime.
...
| Arte/áudio original | P1 | fundação greybox em `15-WORLD-PRESENTATION.md`; skins F0 evoluem por receita (`docs/34`) até P1 autorizar malha/textura pública; linguagem final, checklist jurídico, budgets e produção guiada por `14-ANIMATION-PLAN.md` |
